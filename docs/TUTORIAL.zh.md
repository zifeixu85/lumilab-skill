...
Lumi Lab 的 skill 脚本用 **bun ≥ 1.0** 运行。你**不需要手动装**——两条路径都会自动处理：

- **`install.sh`**：检测到缺 bun 会自动安装（装到 `~/.bun/`，无需 root，约 30 秒）。
- **宿主 chat（OpenClaw / Hermes）**：每个 SKILL.md 顶部有 `## 环境自检` 段，宿主 agent 首次调用前会自己跑环境检查、缺了就装。

纯方法论用法（只让 agent 读 SKILL.md 做对话、不跑脚本）连 bun 都不需要。

可选：`wrangler`（仅 `lumilab deploy` 用）、`qrencode`（仅 `lumilab manage` 二维码用）。
