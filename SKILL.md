...
name: lumilab
description: |
  Lumi Lab —— C 端创业 idea 的快速验证 skills bundle。给一句话 idea，自动跑市场分析 / 竞品扫描 / 人群拆解 / 关键词红蓝海，产出一份图文并茂的网页版分析报告 + 几个具体方向建议，用户选定方向后再自动生成一个能测真实购买意愿的 fake-door 验证页（带 SEO/GEO + 转化追踪），可一键加密部署上线。全程最多问用户 2 次。23 个 skill 协同，跑在 Claude Code / OpenClaw / Cursor / Codex / Hermes / Gemini CLI 里——宿主提供 LLM，bundle 不需要 LLM API key。
...
metadata:
  lumilab:
    ...
    chat_only_ok: true
    default_entry: lumilab-idea-to-landing
    skill_count: 23
  hermes:
...
> 这是 **bundle 入口 SKILL.md**。Lumi Lab 是一套 23 个 skill 的集合，跑在你已有的 AI 宿主里。
