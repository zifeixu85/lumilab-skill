...
## 与 Studio 实时 re-theme 的关系（W4 · 2026-05-30）

这个 picker 定**初始方向**（presets + 3 旋钮 + 品牌色），写 `design_direction.json`。
之后在 **Studio 构建阶段**有一套**扩充的 re-theme 旋钮（~9 个，分组）**，对着**真实落地页 iframe** 即时调：

| 组 | 旋钮 | 写入 token |
|---|---|---|
| 色彩 | 主强调色 / 次强调色 / 表面色调（暖↔冷）| `--accent` / `--accent-2` / `--surface` |
| 形状 | 圆角 / 间距密度 | `--radius` / `--space-scale` |
| 字体 | 标题字体 / 正文字体（白名单，禁 Inter/Roboto）| `--font-heading` / `--font-body` |
| 版式 | hero 版式（居中/分栏/编辑感）/ 按钮风格（实心/描边/胶囊）| `data-hero` / `data-button` |
| 预设 | preset 一键套上面一组默认 | 批量 |

「应用设计」由 server **确定性**写回 `design_direction.json` + landing 的 `theme.css`（不调 LLM）。
本 picker 是「定方向」，Studio re-theme 是「在真页面上微调并持久化」。两者同源 `design_direction.json`。
