 ...
 *   Step 4/5  Tool integrations (all optional)
...
const FONT_LINK = `<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Geist:wght@300..700&family=JetBrains+Mono:wght@400..600&display=swap" rel="stylesheet">`;
...
function progressBar(current: number): string {
  const labels = ["欢迎", "你是谁", "偏好", "工具集成", "部署偏好"];
...
<section class="page-h">
  <h1 class="page-h__title">欢迎</h1>
  <span class="page-h__sub">01 / 05</span>
</section>

<p class="intro">
  <strong>Lumi Lab 是一组 Skills。</strong>你的 AI 宿主（Claude Code、OpenClaw、
  Cursor、Codex、Hermes、Gemini CLI……）已经自带 LLM。这里只需要：
  你是谁、你的偏好，以及——可选的——几个工具 token。
</p>
<p class="intro">
  任何一步都可以跳过。基础 skills（coach、hypothesis ledger、
  content）零 token 也能用——它们走宿主的 LLM。
</p>
<p class="intro" style="color: var(--color-mute);">
  配置写入 <code>~/.lumilab/config.json</code>，token 写入
  <code>~/.lumilab/secrets.json</code>（权限 600，P0 阶段明文存储）。
  之后随时重跑这个向导即可修改。
</p>
...
<section class="page-h">
  ...
  <span class="page-h__sub">02 / 05</span>
</section>

<form id="form-identity" onsubmit="return submitStep2(event)">
  <fieldset>
    <legend>个人资料</legend>
...
async function submitStep2(e) {
  ...
  await postJSON('/api/save', { step: 2, identity: data });
  go('/.setup/3.html');
  return false;
}
`;
  return page("你是谁", 2, body, script);
}
...
<section class="page-h">
  ...
  <span class="page-h__sub">03 / 05</span>
</section>

<form id="form-prefs" onsubmit="return submitStep3(event)">
  <fieldset>
    <legend>写作</legend>
...
async function submitStep3(e) {
  ...
  await postJSON('/api/save', { step: 3, prefs: data });
  go('/.setup/4.html');
  return false;
}
`;
  return page("偏好设置", 3, body, script);
}
...
<section class="page-h">
  ...
  <span class="page-h__sub">04 / 05 · 全部可选</span>
</section>
...
<form id="form-keys" onsubmit="return false">
  ...
  <div class="actions">
    ...
    <button type="button" class="primary" onclick="finishStep4()">下一步 →</button>
  </div>
</form>
`;
  const script = `
...
async function finishStep4() {
  const verifiable = ['cloudflare','exa','tikhub','stripe','resend'];
  const plain      = ['wechat_appid','wechat_secret','x_key','x_secret'];
  const keys = {};
...
async function skipAll() {
  await postJSON('/api/save', { step: 4 });
  go('/.setup/5.html');
}
`;
  return page("工具集成", 4, body, script);
}
...
function renderStep5(cfg: WizardConfig): string {
  ...
  const body = `
<section class="page-h">
  <h1 class="page-h__title">部署偏好</h1>
  <span class="page-h__sub">05 / 05</span>
</section>
...
<form id="form-deploy" onsubmit="return submitStep5(event)">
  <fieldset>
    <legend>分享默认值</legend>
...
async function submitStep5(e) {
  ...
  await postJSON('/api/save', { step: 5, deploy: data });
  await postJSON('/api/done', {});
  go('/.setup/done.html');
  return false;
}
`;
  return page("部署偏好", 5, body, script);
}
...
<p class="intro">
  ...
  <code>/lumilab init</code> 开始第一个 venture，或者随时重跑这个向导修改。
</p>
...
async function handleRequest(req: Request): Promise<Response> {
  ...
  if (path === "/" || path === "/.setup/") {
    const next = Math.min(Math.max(cfg.step + 1, 1), 5);
