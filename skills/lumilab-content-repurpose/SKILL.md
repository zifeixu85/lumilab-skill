...
## 封面图（默认只这 1 张渲染成 PNG）
封面：{title 大字 + 主题图描述} → 用 render-xhs-card.ts 出 content/xhs/<slug>-cover.png

## 配图屏（可选轮播；默认只出封面）
痛点 / 3 个 takeaway / CTA 各一屏 —— **仅当用户要做多图轮播时**才逐张 render-xhs-card.ts；**渲染几屏就出几张图**，绝不能文案写 6 屏却只出 1～2 张（数量必须对得上）。

## 正文（纯文本 + emoji，禁 Markdown）
{4 段：钩子 / 价值 / 细节 / 互动。小红书**不渲染 Markdown** —— 正文里不要用 `**` `##` `-` `>` 反引号 等标记；分点用 emoji（✅🔥💡👇）或直接换行，不用 `-`/`1.` 列表语法}
...
## Self-check
✓ 标题 18 字（≤38）
✓ 渲染的图数 = 文案描述的屏数（默认就 1 张封面）
✓ 首图大字标题：[已确认]
✓ 比例 3:4
✓ 标签 5 个
✓ 正文纯文本 + emoji（无 Markdown 标记）
✓ 无外链
✓ 违禁词扫描：通过
```

### 封面配图：默认 = HTML 模板渲染（AI 出图是可选第二步，非默认）

> **🔒 硬规则：小红书配图默认且只默认走 `render-xhs-card.ts`（HTML 模板 → 截图）。**
> **不要用宿主自己的生图能力（Codex/其它宿主的 image-gen），也不要默认跑 `imagegen.ts`。** AI 生图只在用户**明确**要求（说「AI 出底图 / 要氛围背景图」之类）时才作为第二步增强。
> 原因：HTML 渲染 **0 崩中文字、可控、免费、与 landing 视觉一致**；AI 直出中文大字会崩、不可控、要花钱。看到「做小红书封面」就跑下面第一步，别绕去 AI 生图。

**第一步 · HTML 渲染（默认，确认文案，0 崩字、免费）**——把封面文案/数据填进模板，截图成 1080×1440：
...
- **第二步 · AI 出底图（可选，仅当用户明确要求时才跑；默认跳过）**：
...
❌ 默认用 AI 生成小红书配图（崩中文字、不可控、花钱）—— 默认必须走 `render-xhs-card.ts` HTML 渲染；AI 生图只在用户明确要求时
❌ 在小红书正文里用 Markdown（`**` 加粗 / `##` 标题 / `-` 列表）—— 平台不渲染，会显示成裸符号；正文用纯文本 + emoji
...
✓ 钩子风格 5 平台一致但表达不同
✓ 标签 / 数据 / 引导符合各平台规则
✓ 小红书配图默认 render-xhs-card.ts（HTML）；AI imagegen 仅用户明确要时
✓ 渲染的图数 = 文案描述的屏数（默认 1 张封面，不能写 6 屏只出 1～2 张）
✓ 小红书正文纯文本 + emoji，禁 Markdown 标记
```
...
| 要小红书配图 | **默认** `render-xhs-card.ts` 出封面（HTML 渲染）；**只在用户明确说「AI 出底图 / 氛围图」**才跑 `imagegen.ts`，绝不默认用宿主自己的生图 |
