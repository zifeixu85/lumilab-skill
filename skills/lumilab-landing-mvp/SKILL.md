...
## Landing 语言（出海默认英文 —— 必读）

读 `market_analysis.json` / `project_brief.md` 里的 `landing_lang`：

- **`en`（出海默认）**：landing 全英文，**native-English / 地道表达**，不是中式直译。
  - `<html lang="en">`、`og:locale` 用 `en_US`、`<title>` / `description` / JSON-LD / FAQ 全英文。
  - 文案过 **Anti-Slop EN 禁词**（见下「文案禁用」），并优先用 Phase 1 跑出的英文蓝海关键词做 SEO 锚点与 hero 措辞。
  - CTA 用英文购买类动词：`Pre-order` / `Get early access` / `Reserve — $XX`；modal：`Launching soon — leave your email`。
- **`zh`（国内）**：landing 中文，`<html lang="zh-CN">`、`og:locale` `zh_CN`（本文件其余示例的默认）。
- 缺 `landing_lang` 时：出海 venture 默认 `en`，否则 `zh`。

⚠️ 这是 Lumi Lab 里**唯一**该用英文的产出。Studio dashboard、市场报告、假设、决策、`copy.md` 的注释说明仍用中文 —— **只有 landing 面向访客的可见文案是英文**。
...
- Open Graph 全套：`og:title` `og:description` `og:image` `og:url` `og:type` `og:locale`（随 `landing_lang`：`en` → `en_US`，`zh` → `zh_CN`）
- Twitter Card：`twitter:card`（`summary_large_image`）`twitter:title` `twitter:description` `twitter:image`
- `<link rel="canonical">` 指向页面正式 URL
- 语义化 heading 层级：全页只有一个 `<h1>`，`<h2>`/`<h3>` 逻辑嵌套，不跳级
- `<html lang="…">` 随 `landing_lang`（出海 `en`，国内 `zh-CN`）
...
## theme.css —— 把可调设计暴露为 CSS 变量（W4 · 2026-05-30，必做）

除了 `styles.css`，每个 landing 版本**额外产出 `theme.css`**，`<head>` 在 `styles.css` **之后**引它：

```html
<link rel="stylesheet" href="styles.css">
<link rel="stylesheet" href="theme.css">   <!-- 必须在 styles.css 之后 -->
```

```css
/* theme.css —— Studio 设计面板 POST /api/design/apply 时确定性重写（不调 LLM） */
:root{
  --accent: oklch(45% 0.15 25); --accent-2: oklch(70% 0.12 250);
  --surface: oklch(97% 0.012 85); --ink: oklch(23% 0.02 60); --ink-2: oklch(52% 0.02 70);
  --radius: 2px; --space-scale: 1;
  --font-heading: "Fraunces","Noto Serif SC",Georgia,serif;
  --font-body: "Fraunces","Noto Serif SC",Georgia,serif;
  /* bridge：规范 token → styles.css 实际用的变量名 */
  --color-accent: var(--accent); --color-paper: var(--surface);
  --color-ink: var(--ink); --color-muted: var(--ink-2);
  --color-line: color-mix(in oklch, var(--ink) 14%, transparent);
}
```

要求 `styles.css` 主可见区（hero / CTA / 卡片 / 表单 / 间距 / 字体）**只引用规范 token**：
`var(--radius)`、`var(--font-heading)`、`var(--font-body)`、`calc(... * var(--space-scale))`、
`var(--color-accent)`（经 bridge 来自 `--accent`）。再加 `<body>` 的 `data-hero`（split/centered/editorial）
与 `data-button`（solid/outline/pill）结构钩子。

**为什么**：Studio 构建阶段把真实 landing 放进 iframe，旋钮 `setProperty('--accent', …)` 直接改这些
变量 → **即时可见**；「应用设计」由 server 确定性重写 `theme.css`（不调 LLM）→ 持久化。
没有 theme.css，纯视觉调整就「调了看不出变化」。决赛先覆盖主可见区，长尾样式 Phase 1。
