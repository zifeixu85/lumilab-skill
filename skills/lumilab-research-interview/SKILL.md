...
```
VST: 你已完成 5 次访谈。我对比一下最近 2 次和前 3 次：
     - 痛点重复度：N%
     - 旧方案重复度：N%
     - 触发瞬间重复度：N%
...
VST 在对话内做反模式扫描 + 提取，输出单次结构化 markdown 让用户存档；
不要在聊天里坚持要用户填表单——直接接受流水帐式叙述，VST 负责结构化。
